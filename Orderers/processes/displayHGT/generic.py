...
def getCumulativeCountScaleParams(datafile, maxScale = None, numBands = 3):
    ...
    for band in bandList:
        ...
        for chunk in range(0, chunks):
            ...
            hist, bin_edges = np.histogram(arr, range(0, maxBin + 1))
            print("Histogram size ", hist.size, " standard deviation ", np.std(hist), " and mean ", np.mean(hist))
            histogram = histogram + hist
            print("Cumulated histogram size ", hist.size, " standard deviation ", np.std(hist), " and mean ", np.mean(hist))

        # now work out min and max
        minValue = None
        maxValue = None
        sumValues = 0
        for value in range(0, maxBin):
            sumValues = sumValues + histogram[value]
            #print "Sum of values ", sumValues, " at value ", value
